...
def gudang_barang():
    """
    Halaman utama gudang untuk melihat daftar semua barang.
    ...
    # Ambil semua data barang dari database, urutkan dari yang terbaru
    cursor = mysql.connection.cursor()
    cursor.execute("SELECT * FROM gudang ORDER BY id_gudang DESC")
    data_barang = cursor.fetchall()  # Ambil semua data
    cursor.close()
    
    # Tampilkan halaman gudang dengan data barang
    return render_template('gudang/barang.html', barang=data_barang)
